...
(function ($) {
    ...
    function initActiveMenu() {
        ...
        $(".leftbar-tab-menu a, .left-sidenav a").each(function () {
            ...
            if (localStorage.getItem('lastSelectedMenuItem') != null && localStorage.getItem('lastSelectedMenuItem') != '') {
                pageUrl = localStorage.getItem('lastSelectedMenuItem');
            ...
            if (this.href == pageUrl) {
                ...
                $(".leftbar-tab-menu a, .left-sidenav a").each(function () {
                    let lastSelectedMenuItem = localStorage.getItem('lastSelectedMenuItem');
                    if (this.href == lastSelectedMenuItem) {
                        $(this).addClass("active");
                        $(this).parent().addClass("active"); // add active to li of the current link                 
                        $(this).parent().parent().addClass("in");
                        $(this).parent().parent().addClass("mm-show");
    ...
    function initAutoComplate() {
        ...
        //    $.get('/CommonEntity/Search', { data: $(this).val() }).done(function (result) {
    ...
    function getSearchResults(data) {
        ...
        return new Promise(function (resolve, reject) {
            $.get('/CommonEntity/Search', { data: data }).done(function (result) {
                resolve(result);
            }).fail(function (err) {
                reject(['Məlumat tapılmadı']);
            });
        });
    }
    ...
    $('#AllCompo').keyup(function () {
        if ($(this).val().length != 0) {
            getSearchResults($('#AllCompo').val()).then((data) => { showResults(data); }).catch(() => { return ['Məlumat tapılmadı'] })
        }
